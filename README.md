...
- **首页**：本周新番时间表（Bangumi `/calendar`）+ 继续观看（断点百分比进度卡片，单项移除/一键续播）+ 我的追番（本地收藏与管理、**并发检查追更状态、动态呈现「更新至第 X 集」更新徽标**）+ 最近搜索（单项删除/清空/SQLite 持久化，跨重启）
...
- **播放器与连播体验**：全屏沉浸 OSD 浮层（快进快退/音量/静音/倍速/连播倒计时/弹幕调谐全量视觉反馈）、**播放信息统计面板（Stats for Nerds，快捷键 `I` 调出，实时显示协议/分辨率/视口尺寸/丢帧率/缓冲比例/弹幕状态）**、播放器内嵌磨砂缓冲旋转 Spinner、沉浸播放鼠标停滞 2.5s 自动休眠（隐藏光标）、断点续播、连播下一集（顶栏「下一集 ⏭」按钮 / `N` 快捷键 / 播毕 3 秒倒计时自动切集）、倍速/音量记忆、全套桌面快捷键（`/` 聚焦搜索、`Escape` 退出返回、`Space` / `←→` 或 `JL` / `↑↓` / `M` 静音 / `0`~`9` 进度跳转 / `<` `>` 倍速 / `F` / `N` / `I` / `[` `]`）、点击暂停、双击全屏、HLS 出错自动恢复
...
- **离线缓存（M5 HTTP 引擎）**：候选列表点「⤓ 缓存」把 Jellyfin/HLS/直链视频下载到本地（m3u8 自动选最高码率、全量分段下载、本地重写 playlist），经 `anicache://` 自定义协议回放（支持 Range 拖动 seek）；设置 → 缓存 里管理（**一键打开缓存根目录、一键清空全部缓存与记录**、播放/定位/删除）；暂不支持加密流（AES-128）与 BYTERANGE 分段
