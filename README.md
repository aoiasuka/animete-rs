...
- **与好友一起看（Syncplay / Watch Together 协同观影系统）**：**对标 Ani 6.0 旗舰协同特性**，基于 Rust 原生轻量化网络广播服务（基于 `tokio::net::TcpListener` + HTTP SSE 与 REST API，零外部重依赖，支持局域网、WiFi、Tailscale、ZeroTier、VPN）；房主一键创建房间并智能探测显示可用局域网 IP 与邀请码（如 `192.168.1.105:19280`），好友秒级加入；**毫秒级播放、暂停、跳转（Seek）跨端同步**，带 **智能微调倍速平滑防卡顿追赶算法（Drift Compensation）**；播放器内嵌悬浮协同抽屉（快捷键 `Y`），展示在线成员延迟与对齐状态、房主切集感应卡片与一键同步跳转、**房间聊天实时转化为金色边框专属高光弹幕**，以及 **名场面表情共鸣（🎉、❤️、😂、🔥、👏 全屏气泡浮空粒子动画）**
- **本地视频与下载目录智能扫描（Local Media Scanner）**：顶栏新增「本地 📂」快捷入口，深度扫描本地已下载视频目录（支持 .mp4, .mkv, .webm, .avi）；**内置智能文件名解析器，精准剥离字幕组/压制参数标签并提取动画原名与集数**；支持一键直连本地流播协议秒开，并自动根据解析结果发起弹弹play 全网弹幕秒配与 Bangumi 关联
...
- **在线播放**：
  ...
  - **画面缩放赏析与拖拽平移模式（Video Zoom & Pan Studio）**：控制栏「画面 📺」菜单新增 100% ~ 300% 画面缩放（100% / 125% / 150% / 200% / 300%），支持快捷键 `Ctrl++` 局部放大、`Ctrl+-` 缩小与 `Ctrl+0` 一键复原；放大状态下自动开启**鼠标按住画面自由拖拽平移（Drag-to-Pan）**，顶部浮现精致毛玻璃 HUD 胶囊（如 `🔍 150% 拖拽平移中`），点击胶囊或重置按钮瞬间恢复 100% 原始尺寸，满足作画细节赏析与背景微距探查；
  - **控制栏上一集/下一集快捷导航与剧集名悬浮感知（Quick Episode Navigation Buttons）**：控制栏新增「⏮ 上集」与「下集 ⏭」直达按钮组（快捷键 `B` / `N`）；智能感知前后集有效性（处于首集或末集时自动平滑置灰与防误触），鼠标悬浮直观预览上一集/下一集的集数与完整单集标题；
  - **全剧完结撒花与通关成就纪念卡片（Series Completion Celebration Card）**：播完番剧最后一集时，播放器中央弹出精致金色光芒完结撒花卡片；实时统计并展示番名与总话数补完纪念，提供「⭐ Bangumi 评分评价」一键直达打分、「📸 2x 完结打卡海报」一键生成高清打卡卡片，以及「🔄 从头重播」一键重温；
  - **选集抽屉「📸 相册」画廊与名场面批处理系统（Screenshot Moments Gallery & Batch Actions）**：选集抽屉新增「📸 相册」独立视图，全自动汇总当前观影会话截取的全部名场面画面；每张截图呈现高清微缩图、真实分辨率、截屏模式与精确时间戳徽标（点击时间戳直接跳转 Seek 该画面）；支持卡片内**一键复制图像到剪贴板**、**单张下载 PNG**、**一键转存为名场面打点书签 💎**、**单项移除**，并支持**「全部保存 💾」批量导出**与**一键清空相册**；点击缩略图呼出原图高清放大视窗（Lightbox），支持 Esc 快速关闭；
  - **时间轴悬浮视频微缩画面实时预览（Timeline Video Thumbnail Scrubbing Preview）**：鼠标在交互时间轴与热力图轨道滑动时，动态弹出精致毛玻璃 16:9 高清视频微缩快照卡片（`160×90px`）；内置离屏硬件加速抓帧与智能重绘引擎，超低开销跟随光标，具备智能左右边缘自适应贴边防裁剪；卡片内实时呈现当前帧秒级真实画面、精确时间戳（`05:24 / 24:00`）、百分比进度，并结合剧情上下文智能投射「🔥 名场面高能」、「💎 名场面打点书签」与「🎵 片头曲 OP / 🎶 片尾曲 ED」专属动态徽章；
  - **智能片头片尾时间轴可视化标记（Smart OP / ED Visual Timeline Chapter Marks）**：时间轴轨道智能投射青蓝霓虹「OP」片头分段高光条与暖橘玫瑰「ED」片尾分段高光条；区间边界直观呈现，支持悬浮秒知时长，点击 OP 分段一键无缝跳过片头，点击 ED 分段秒级直达剧末片尾曲；
  - **外挂字幕高级排版与字体工坊（Subtitle Typography Studio）**：外挂字幕菜单深度升级为排版工坊，原生 WebVTT 渲染链路全面打通：
    - **中文字体自由切换**：支持默认系统无衬线、思源黑体（现代番剧风）、霞鹜文楷（文艺典雅风）、微软雅黑、圆体幼圆（轻小说治愈风）；
    - **字间距与行距精细调谐**：支持 0px / +1px / +2px / +4px 字距自选，紧凑 1.2 / 标准 1.4 / 舒展 1.7 行间距调节；
    - **4 种立体光影质感**：标准纯黑阴影、加厚纯黑重描边、柔光环境氛围外发光、院线胶片立体投影；
    - **一键排版风格预设**：内置「🌟 动漫新番」（思源黑体+纯白+加厚重边）、「🏮 典雅文楷」（霞鹜文楷+明黄+柔光）、「🎬 院线电影」（圆体+纯白+胶片投影+2px字距），即点即用，全量排版偏好跨会话自动持久化；
  ...
  - **播放中无缝换源面板（In-Player Source Switcher）**：快捷键 `Shift+S` 或控制栏「换源 🔄」，播放中呼出片源切换面板，自动列出当前动画剧集的所有候选源（包含本地离线缓存、各字幕组、清晰度与文件体积）；点击任意候选源**自动继承当前播放进度 `currentTime` 秒级无缝续播**，彻底解决看番中途卡顿或内嵌字幕不合心意时需退回搜索的割裂体验；
  - **夜间看番人声动态范围压缩（Night Mode Dialog DRC & Dynamic Compressor）**：基于 Web Audio `DynamicsCompressorNode` 构建动漫专用声学压限拓扑（-24dB 软拐点、12:1 压缩比、0.003s 极速瞬态捕获）；控制栏「画面 📺」一键启闭，在深夜微小音量下大幅清晰提亮低语台词与角色对白，同时平滑压制突发爆炸与尖锐枪声，深夜观影告别手忙脚乱调音量；
  - **名场面高能打点书签系统（Scene Bookmarks & Moments Marker）**：快捷键 `M` 或控制栏「打点 💎」，在当前播放秒数快速标记高能瞬间；进度条精准投射高亮宝石标记（💎），悬停显示打点名与时间，点击秒级精准 Seek；在选集抽屉「💎 名场面」Tab 中集中展示打点清单，支持一键跳转、重命名、删除及一键导出为 Markdown 观影笔记并复制到剪贴板；后端通过 SQLite `scene_bookmark` 表实现跨会话持久化存储；
  - **名场面无损截帧与合成升级（纯原画 / 外挂字幕 / 弹幕全合成）及悬浮微缩卡片**：快捷键 `C` 或「截屏 📸」，支持三种合成导出模式（纯净原画、包含高清外挂双语字幕、包含字幕+弹幕全屏合成）；截取后在右下角弹出精致磨砂悬浮预览卡片，显示截取缩略图、分辨率与时间戳，支持一键再次复制到剪贴板与下载 PNG；
  - **弹幕高能热词洞察与峰值时刻精准跳转（Danmaku Keyword Insights）**：弹幕引擎载入后自动分析当集高频二次元流行名梗（"草"、"前方高能"、"awsl"、"神回"、"泪目"、"名场面"、"起立"、"完结撒花"等），在高级调谐面板以火焰徽章胶囊动态陈列热词频次；点击任意热词自动将进度精准定位至该热词密集的剧情峰值时刻；
  - **弹幕二次元高能名梗与快捷反应表情发射台（Danmaku Quick Memes & Reaction Emotes Floating Palette）**：播放器弹幕条新增「✨ 名梗」入口（快捷键 `Alt+M`），呼出精致毛玻璃高能梗分类展台（包含「⚡ 高能助威」前方高能/全体起立/战歌起/燃起来了、「🌟 名台词与名梗」草/awsl/神回/世界名画/心肺停止/请收下我的膝盖、「💖 打卡共鸣」打卡/泪目/完结撒花/圣地巡礼/二刷等）；支持「即点即发 🚀」一键秒发与填入输入框两种模式，点击伴随 OSD 视觉反馈与全屏飞屏动画；
  - **番剧专属片尾跳过点与时间轴章节精准联动（Per-Anime Custom ED Timeline Pinning & Skip Points）**：除专属片头点外，「画面 📺」菜单新增「专属片尾点」标记；播放至片尾曲 ED 起点时点击「📌 设为片尾点」，精准记忆当前番剧专属片尾触发时刻（`ani_ed_skip_{id}`）；时间轴自动同步呈现精准 ED 分段高光条，并在连播与跳过片尾开启时秒级精准触发自动切集，完美解决不同动画片尾时长各异及片尾彩蛋避让难题；
  - **弹幕人物脸部智能避让保护与轨道碰撞预防（Smart Danmaku Center Face Shield & Anti-Collision Flow）**：弹幕调谐面板新增「👤 防挡人物」模式，智能保护画面中央 35% 核心主体人物视区，将滚动弹幕上下动态分流，防止人物面部特写被密密麻麻的文字遮挡；重构弹幕进轨时间窗算法，精准计算弹幕字长与入屏速度，消除后车追尾与文字重叠，大幅提升超高密度弹幕下的视觉流畅度；
  - **色彩滤镜工坊扩展与时间轴快退快进微调按键（Cinema/Cyber/Vintage Filters & Jump Buttons）**：色彩滤镜扩充「胶片院线」（细腻对比度+微红暖调）、「激燃战斗」（冷艳高饱和+战斗视效强化）与「复古赛璐珞」（90年代经典 OVA 黄金时代胶片质感）；控制栏集成「⏪ 5s」与「5s ⏩」快捷微调按键，支持键盘 `J` / `L`（±10s）与方向键无缝联动；
  - **弹幕时间轴微调胶囊与番剧维度自动记忆**：控制栏增加可视化的「⏱️ 弹幕微调」胶囊，支持 ±0.1s / ±0.5s / ±1.0s 步长微调与一键重置对齐，自动以番剧与视频维度记忆时间轴偏移量，下次播放同一番剧自动恢复最佳对齐；
  - **实时弹幕池抽屉、即时检索与一键关键词屏蔽（Interactive Danmaku Pool Drawer & Shield Studio）**：选集抽屉新增「💬 弹幕」独立视图，实时呈现当前剧集全量弹幕总数与详细列表；支持输入任意关键词模糊检索、按弹幕类型（全部 / 滚动 / 顶端 / 底端 / 彩色 / 高频梗）一键过滤，并支持按时间升降序与热度多维排序；每条弹幕支持**点击时间戳秒级定位 Seek**、**一键复制弹幕内容**以及**一键屏蔽此词（🚫 One-Click Shield，动态追加至屏蔽词库，全屏实时净化并持久化）**；支持「📍 随播定位」时钟同步平滑滚动高亮当前弹幕，支持一键导出当前弹幕为 JSON / 文本清单；
  - **连播模式精准联动、可定制倒计时与片尾浮动交互卡片（Customizable Auto-Next Countdown & Playback Modes）**：循环模式深度协调（顺序连播 ⏭、单集循环 🔂、列表循环 🔁、播完即止 ⏹，快捷键 `R`）；「画面 📺」菜单支持**连播倒计时自定义（0s 立即秒连 / 3s / 5s 默认 / 10s / 关闭自动连播）**；临近片尾时弹出毛玻璃交互卡片，显示下一集标题、平滑倒计时进度条，支持随时点击「▶ 立即播放」或「✕ 取消连播继续看彩蛋」；
  - **Web Audio 对白人声清透增强与耳机左右声道平衡（Vocal Clarity Dialogue Booster & Stereo Balance Panner）**：在 Web Audio 音频链路中新增高精度参量人声中频峰值滤波器（2800Hz，Q=1.2）与 `StereoPannerNode` 声道平衡节点；「画面 📺」菜单新增**人声清透增强（原声 0dB / 微亮 +3dB / 清透 +6dB / 极清 +9dB）**，在激烈战斗爆炸或背景音乐混杂的老番中瞬间凸显声优对白台词；支持 **-1.0 (左耳 L) ~ 0 (居中 C) ~ +1.0 (右耳 R) 精细声道平衡微调与一键重置**，完美拯救单侧偏音或听力差异；
  - **高精度逐帧步进与作画赏析系统（Frame-by-Frame Stepping & Sakuga Studio）**：播放器集成「⏮ 帧」与「帧 ⏭」步进按钮组（快捷键 `,` 后退 / `.` 前进，`Ctrl+←` / `Ctrl+→`，`Shift+,` / `Shift+.` 高能 ±5 帧微调），播放中触发自动平滑暂停；在「画面 📺」菜单支持配置**动漫原生 24fps 作画基准**、30fps 与 60fps；中央 HUD 弹出毫秒级时间戳（如 `🎞️ 逐帧微调: +1 帧 · 14:22.042 (24fps)`），精准定位打斗关键帧，与截图工具（`C`）无缝联动一键无损截取原画；
  - **交互式倍速悬浮面板、无级变速与原声音调保持（Interactive Speed Studio）**：控制栏「倍速」升级为悬浮调谐面板，集成 10 档预设矩阵（0.5x ~ 4.0x）与 `0.2x ~ 4.0x` 平滑无级变速滑块；支持「原声音调保持 (Preserve Pitch)」开关，倍速播放下对白声调依然原汁原味，关闭时可体验磁带机械变调特效；
  - **选集抽屉即时过滤与播放集数平滑居中（Episode Drawer Live Search & Auto-Centering）**：选集抽屉内置即时检索栏，输入集数（如 "12"、"01"）或分类关键词（"特别篇"、"OVA"、"剧场版"）秒级过滤；呼出选集抽屉时自动平滑滚动并将当前播放中的集数居中对齐，告别长篇多季动画繁琐的手动翻找；
  - **倍速平滑微调与中央 HUD 动态反馈**：快捷键 `Shift+<` / `Shift+>` 支持 ±0.1x 平滑微调（0.2x ~ 4.0x），播放器中央弹出顺滑的半透明 HUD 动画；
  - **追番全景统计看板增强与追番成就战报导出（Anime Bento Wrapped）**：顶栏常驻「统计 📊」入口（快捷键 `Shift+A`），深度聚合累计观影时长、看毕话数、90天连击热力图；新增**动漫题材偏好分布（奇幻/热血/日常/科幻/恋爱/悬疑等维度统计占比）**与**一键导出追番战报 (Anime Bento Card)**，格式化输出精美成就卡片并写入剪贴板；
  ...
  - **桌面置顶与极简画中画伴播模式（Desktop Window Pinning & Mini Companion Mode）**：顶栏控件与播放控制栏集成「置顶 📌」（快捷键 `Shift+P`），基于 Tauri 2 原生窗口 API 始终将播放视窗保持在所有桌面应用最前端；新增「伴播 🗖」（快捷键 `Shift+M`），一键无感切换为 540×340 黄金 16:9 迷你悬浮伴播小窗，自动启用全局置顶、隐藏页面顶栏与操作栏（控制条鼠标悬停自动浮现）；右上角常驻「⛶ 还原」悬浮胶囊或按 `Esc` 即可无损复原原窗口尺寸与常规布局，工作摸鱼边看番两不误；
  - **多音轨声轨选择与切换系统（Multi-Track Audio Switcher）**：控制栏新增「音轨 🎧」菜单，全自动探测 HTML5 `video.audioTracks` 与 HLS.js `hls.audioTracks`，智能解析语言属性并优雅标注国旗与语言标（🇯🇵 日语原声 / 🇨🇳 中文配音 / 🇺🇸 英语 / 🇭🇰 粤语 / 🎙️ 评论音轨）；点击即可秒级切换声轨，并伴随沉浸 HUD 视觉反馈；
  - **弹幕同屏密度温控与 4 档字号定制（Danmaku Density Governor & Scaling）**：弹幕调谐面板内置 4 档同屏密度温控（`不限 120+` / `密集 80条` / `适中 40条` / `清爽 20条`），在 OP/ED 与高能峰值爆发期自动平滑限流控温，防止铺天盖地遮挡关键画面；字号扩充为 4 档自选（小 18px / 中 24px / 大 30px / 巨 36px），完美契合小窗伴播与大屏 4K 观影；
  ...
  - **A-B 片段循环时间轴可视化高光标记与微调控制器 (Visual A-B Loop Timeline Markers & Mini Repeat Controller)**：标记 A/B 点时（快捷键 `[` / `]`），时间轴轨道精准投射青蓝与玫红高光旗标（A / B）与区间发光范围条；支持点击旗标与范围条秒级跳转；画面上方弹出精致浮动控制面板，实时统计循环次数（`🔁 A-B 循环中 (第 X 次)`），提供 `A-1s` / `A+1s` / `B-1s` / `B+1s` 与快捷键 `Ctrl+[` / `Ctrl+]` 精细微调端点，支持 `\` 一键解除；时间轴微缩悬浮预览卡片智能识别并高亮投射「🔁 A-B 循环区间」动态徽章；
  - **断点续播智能交互胶囊与一键从头播放 (Resume Playback Interactive Toast & Replay Action)**：恢复历史播放进度时，播放器左下角浮现毛玻璃交互胶囊（`📍 已为您恢复至上次播放进度 12:34`），提供「↺ 从头播放」一键秒级重返 00:00，彻底消除想重温开头却被自动跳后的痛点，7 秒后平滑淡出；
  - **动漫原画线条锐化与超清增强着色器滤镜 (Anime Line Crisp & Edge Sharpening Shaders)**：控制栏「画面 📺」菜单新增「线条锐化」与「超清增强」两档基于 GPU 硬件加速 3×3 卷积矩阵（`feConvolveMatrix`）的动漫专用边缘轮廓重塑着色器滤镜，大幅消除 720p/1080p 在 2K/4K 高清大屏上放大的线条发虚与微模糊感，角色轮廓锐利坚挺；
  - **播放器原生音频律动动态频谱仪 (Cyber Anime Audio Spectrum Visualizer)**：在 Web Audio 链路无损接入 `AnalyserNode` 32 频段 FFT 频谱分析器；控制栏「画面 📺」一键启闭，在画面底部投射随音乐节奏起伏的霓虹渐变（电光紫 ➔ 樱花粉 ➔ 天空蓝）律动频谱柱与平滑跌落峰值帽（Peak Caps），为动漫 OP/ED 与高燃战斗曲带来赛博沉浸视听体验；
  - **画面比例与画质视效**：支持自适应、16:9、老番 4:3 比例修复、21:9 超宽屏、拉伸铺满 Fill、去黑边裁剪 Cover，支持左右镜像翻转与顺时针 90° 旋转，快捷键 `W` 一键循环切换；
  - **二次元动漫色彩滤镜**：内置硬件加速滤镜引擎（原画 / 动漫鲜艳 / 护眼柔和 / 明亮锐利 / 暗部增强 / 胶片院线 / 激燃战斗 / 复古赛璐珞 / 线条锐化 / 超清增强 / 黑白漫画），跨剧集持久化记忆；
  - **智能跳过片头/片尾与 A-B 循环**：开播 0.5s~15s 悬浮跳过胶囊提示、一键快进 OP（快捷键 `S`）、连播自动跳过 OP；**支持自定义片头/片尾跳过时长（60s / 80s / 85s / 90s / 100s / 120s 快速自选）**；**智能跳过片尾（Auto-Skip ED）尾声自动无缝切换下一集**；PotPlayer 风格 A-B 片段复读循环（`[` 设起点 A、`]` 设终点 B、`\` 一键清除、`Ctrl+[` `Ctrl+]` 微调）；
...
- **弹幕高能热力波形与即时发射台**：
  ...
  - **弹幕专业调谐与手动搜索匹配（Manual Match）**：在设置页填入免费申请的 AppId/AppSecret 后，播放时按「番名 + 集号」自动匹配弹幕池；**新增「🔍 手动搜索匹配弹幕源」浮窗，彻底解决番剧译名不一致、OVA/特别篇或多季动画无法自动匹配的痛点，支持输入任意关键词即时检索并一键绑定载入**；支持直接导入或拖拽挂载本地 .xml（Bilibili/弹弹play 标准格式）与 .json 弹幕文件；**支持 2K/4K/Retina 高清屏物理像素 DPR 自适应极清绘制，文字边缘锐利无锯齿**；结果缓存 3 天（SQLite，网络失败时回落过期缓存），后端完成去重 + 关键词/用户/类型屏蔽；**控制栏弹幕高级调谐菜单（快捷键 `D`），支持 3 档显示区域、4 档字号缩放、3 档飘字速度、不透明度快速切换与时间轴微调校准（`-` / `=`）**；全屏 Canvas 图层跟随，轨道满自动限流，seek 零延迟重定位；
  - **弹幕字体工坊与描边黑度调谐（Danmaku Typography Studio）**：弹幕渲染层深度打通多字体排版与自定义描边：
    - **中文字体自选**：默认系统、思源黑体（现代番剧风）、霞鹜文楷（古风文艺）、醒目粗黑（大字视觉冲击）、圆体幼圆（轻小说治愈）；
    - **描边黑度三档调节**：轻描边 (2px)、标准描边 (3px)、重描边 (5px)，在高亮度背景下文字边缘依然坚挺清晰不失真；
    - **字重自选**：常规 (600) 与特粗 (800)，全量排版偏好跨会话自动持久化；
